...
echo "=========================================================================="
echo "   BENCHMARK VTUNE (GPU Hotspots & Memory / Cache Characterization)"
echo "=========================================================================="
...
echo "[vtune] Collection GPU Hotspots (Overview mode, sampler & cache accesses)..."
"$VTUNE_BIN" -collect gpu-hotspots -knob characterization-mode=overview -start-paused -result-dir "$PROJECT_DIR/r@@@gpu_hs" -- env TMP_DIR="$TMP_DIR" ./scripts/interactive_runner.sh "$APP_BIN"

RES_DIR=$(ls -td "$PROJECT_DIR"/r*gpu_hs* 2>/dev/null | head -n1 || true)

if [ -n "$RES_DIR" ] && [ -d "$RES_DIR" ]; then
	echo ""
	echo "=========================================================================="
	echo "📊 TOP GPU COMPUTE KERNELS & HARDWARE UTILIZATION"
	echo "=========================================================================="
	SUMMARY_FILE="$OUT_DIR/vtune_gpu_hotspots_summary.txt"
